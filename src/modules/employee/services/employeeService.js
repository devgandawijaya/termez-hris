/**
 * Employee Service - Extended Enterprise Employee Management
 * Includes CRUD operations, bulk insert, and NIK validation
 */
...
// In-memory employee store (singleton pattern)
let employeeStore = null;

/**
 * Initialize employee store with dummy data
 */
const initializeStore = () => {
  if (employeeStore) return employeeStore;
  
  employeeStore = generateDummyEmployees(50);
...
const generateDummyEmployees = (count = 50) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    employees.push({
      ...
      fullName,
      nik: 'NIK' + String(100000 + i).padStart(6, '0'),
...
export const createEmployee = async (data) => {
  ...
  const newEmployee = {
    ...
    employmentType: data.employmentType || data.type || EmploymentType.PERMANENT,
    jobLevel: data.jobLevel || JobLevel.JUNIOR,
    workLocation: data.workLocation || data.location || WorkLocation.JAKARTA,
...
export const bulkInsertEmployees = async (employees) => {
  ...
  for (let i = 0; i < employees.length; i++) {
    try {
      ...
      const existingNIK = employeeStore.find(e => e.nik === emp.nik);
      if (existingNIK) {
        errors.push({
          row: i + 1,
          data: emp,
      ...
      const newEmployee = {
        ...emp,
        id: generateId(),
        employeeId: 'EMP' + String(employeeStore.length + 1000 + inserted.length).padStart(4, '0'),
        employmentStatus: emp.status === 'Inactive' ? EmploymentStatus.INACTIVE : EmploymentStatus.ACTIVE,
        basicSalary: parseInt(emp.salary) || 0,
        allowances: 0,
        gender: Gender.MALE,
        maritalStatus: MaritalStatus.SINGLE,
        employmentType: emp.employmentType || emp.type || EmploymentType.PERMANENT,
        jobLevel: emp.jobLevel || JobLevel.JUNIOR,
        workLocation: emp.workLocation || emp.location || WorkLocation.JAKARTA,
        joinDate: emp.joinDate || new Date().toISOString().split('T')[0]
      };
      
      employeeStore.unshift(newEmployee);
      inserted.push(newEmployee);
    } catch (err) {
      errors.push({
        row: i + 1,
        data: employees[i],
...
export const checkNIKExists = async (nik, excludeId = null) => {
  ...
  const exists = employeeStore.some(emp => 
...
export const getFilterOptions = async () => {
  ...
  return {
    departments: [...new Set(employeeStore.map(e => e.department))],
    statuses: [...new Set(employeeStore.map(e => e.employmentStatus))],
    locations: [...new Set(employeeStore.map(e => e.workLocation))],
    employmentTypes: [...new Set(employeeStore.map(e => e.employmentType))]
  };
};

/**
 * Get all employees (for validation checks)
 */
